...
void draw(){
  ...
  if(flagPort){
    SerialComm();
    if(!flagJugadores && !flagGame && !flagGameEnd){
      playintro();
      stringVal="I";
    }
    if(flagJugadores){
...
void SerialComm(){
  if(inString!=null){
    if(inString.equals("S")){
      //println("Flagplayer 2 activated, string read S"+inString);
      flagPlayer2=true;
      //stringVal="G";
    }
  ...
  if(frameCount%30==0){
    myPort.write(stringVal);
    println("Se escribió el valor" + stringVal);
  }
...
void getPoints(){
  ...
  if(inString!=null){
    ...
    if(inString.length()>9){
      ...
      //println(index+"-"+inString);
    }
    if(index>0){
      //println("P1 "+inString.substring(0,index)+" , P2 "+inString.substring(index+1,inString.length()-1));
      //println(inString.substring(0,index));
      P1Points=int(inString.substring(0,index));
      //println(inString.substring(index+1,inString.length()-1));
      P2Points=int(inString.substring(index+1,inString.length()-1));
      if(P2Points==0){
        Points=P1Points;  
      }else{
        if(P1Points<0){P1Points=0;}
        if(P2Points<0){P2Points=0;}
        Points=floor((P1Points+P2Points)/2);
      }
...
void Game(){
  ...
  if(Timer>0){
    ...
    fill(PHYellow);
    text(str(Points), 0, rowGame1, width, bigtextgame*2);
    String puntos;
...
void playoutro(){
  ...
  println(outro.time() + " " + outro.duration());
  if(outro.time()+5.15 >= outro.duration()){
    outro.jump(0);
    resetPlayers();
    flagGameEnd=false;
    intro.play();
